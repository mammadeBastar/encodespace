...
import * as v2 from './modules/vec2.jsx'
...
function updateView(cursor, context, data) {
	...
	if (cursor.pressed) {
		...
		targetScale = {
			y: 1.01*WIDTH / context.cols / context.metrics.aspect,
			x: 1.01*WIDTH / context.cols,
		};
	}

	if (data.viewScale.y !== targetScale.y || data.viewScale.x !== targetScale.x) {
		data.viewScale.y += 0.01 * (targetScale.y - data.viewScale.y);
		data.viewScale.x += 0.01 * (targetScale.x - data.viewScale.x);
	}
	...
	if (data.viewFocus.y !== targetFocus.y || data.viewFocus.x !== targetFocus.x) {
		data.viewFocus.y += 0.01 * (targetFocus.y - data.viewFocus.y);
		data.viewFocus.x += 0.01 * (targetFocus.x - data.viewFocus.x);
	}
}
